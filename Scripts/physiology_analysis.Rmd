...
This RMarkdown is the analysis for "Conspecific interactions between corals mediate the effect of submarine groundwater discharge on coral physiology", by J.R. Kerlin, D.M. Barnas, and N.J. Silbiger.
...
We have growth data and metabolism data ready from my experiment and water biogeochemistry data.
...
In this analysis, we are looking at 2 metabolism rates and 2 endosymbiont physiology values:  
- gross photosynthesis & respiration  
- endosymbiont density & chlorophyll a content  

This experiment was deployed (8/3-8/17/2021).
There seemed to be some outliers in the CT temp data, so we filtered out temps over 32C.
...
# Here is information on the plate collection where we look at which fragments were missing
...
We clean up the data for which fragments were missing when we collected the plates.  
...
We combine the coral response variable dataset with the SGD biogeochemistry dataset + the "fragments" df.
We need one in long format and one in wide format for the following analyses.
From this, we have a total of 59 fragments that are being analyzed, compared to the 72 fragments we would have analyzed if we did not remove the fragments that were missing competitors.
...
Next, we create separate dataframes for each response variable.
...
### AIC plots
```{r aic_patched, fig.height=6, fig.width = 7, dev="pdf"}
...
```{r control1_patched, fig.height=6, fig.width = 7, dev = "pdf"}
gp_plot_control1 <- ggplot(GP_plot_control, mapping = aes(x = range_pH, y = Final)) +
...
```{r control2_patched, fig.height=6, fig.width = 7, dev = "pdf"}
gp_plot_control2 <- ggplot(GP_plot_control, mapping = aes(x = range_pH, y = Final)) +
...
```{r residual_patched, fig.height=6, fig.width = 7, dev = "pdf"}
gp_plot_resid <- ggplot(tidy_gp1, mapping = aes(x = Treatment, y = estimate)) +
...
# Calculate some summary values for interpretation

```{r}
# Endosymbiont density across min N gradient
data_wide %>%
  filter(Response_measurement == "ED",
         Treatment == "No_neighbor",
         min_NN_umolL == min(min_NN_umolL, na.rm = TRUE)|min_NN_umolL == max(min_NN_umolL, na.rm = TRUE)) %>%
  select(Treatment, Final,min_NN_umolL )


# Endosymbiont density across pH Range gradient
data_wide %>%
  filter(Response_measurement == "CCcm2",
         Treatment == "No_neighbor") %>%
  select(Treatment, Final,range_pH ) %>%
    filter(range_pH == min(range_pH, na.rm = TRUE)|range_pH == max(range_pH, na.rm = TRUE)
         ) 
...
data_wide %>%
  group_by(Treatment, Response_measurement) %>%
  summarise(mean_value = mean(Final, na.rm = TRUE)) %>%
  pivot_wider(names_from = Response_measurement,
