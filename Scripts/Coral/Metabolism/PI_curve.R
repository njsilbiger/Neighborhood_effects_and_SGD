...
#Load Sample Info
Sample.Info <- read_csv(here("Data_Raw", 
                             "Coral",
                             "Metabolism",
                             "PI_curve",
                             "Metadata",
...
sa_volume <- read_csv(here("Data_Raw", 
                           "Coral",
                           "Metabolism",
                           "PI_curve",
                           "Metadata",
                           "sample_sa_volume.csv"))
...
for(i in 1:length(file.names.full)) { # for every file in list calculate O2 uptake or release rate and add the data to the Photo.R dataframe
  ...
  pdf(paste0(here("Output", "Coral", "Metabolism", "PI_curve", "RespoOutput"),rename,"thinning.pdf")) # open the graphics device
...
ggsave('Output/Coral/Metabolism/PI_Curve/RespirationRates.png')
...
dev.copy(png,'Output/Coral/Metabolism/PI_Curve/PICurve.png', width = 5, height = 4, units = "in", res = 1200)
dev.off()
...
pdf("Output/Coral/Metabolism/PI_Curve/PIcurve.pdf")
